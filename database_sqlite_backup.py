...
# Эти константы должны совпадать с config.py
# Временно определяем их здесь для работы без импорта
BONUS_WAGER_MULTIPLIER = 3  # нужно сыграть в 3 раза больше бонуса

# ========== ЗАГЛУШКИ ДЛЯ СОВМЕСТИМОСТИ ==========
async def init_db_pool():
    """Для SQLite возвращаем None (не используется)."""
    return None
...
async def get_daily_withdrawn(user_id: int) -> int:
    """Возвращает сумму выведенных баллов пользователем сегодня."""
    async def _get():
        async with aiosqlite.connect(DB_NAME) as db:
...
async def get_last_deposit_time(user_id: int) -> int:
    """Возвращает время последнего пополнения."""
    async def _get():
        async with aiosqlite.connect(DB_NAME) as db:
            cursor = await db.execute(
...
async def get_pending_withdraw_count(user_id: int) -> int:
    """Возвращает количество ожидающих заявок на вывод."""
    async def _get():
        async with aiosqlite.connect(DB_NAME) as db:
            cursor = await db.execute(
...
async def get_daily_total_withdrawn_rub() -> float:
    ...
    async def _get():
        async with aiosqlite.connect(DB_NAME) as db:
            today_start = int(time.time()) - (int(time.time()) % 86400)
            # amount_rub нет в таблице, считаем через amount_usdt
            cursor = await db.execute(
...
async def get_bonus_wagering_status(user_id: int) -> Tuple[int, int, bool]:
    """Возвращает (bonus_balance, bonus_wagered, is_cleared)."""
    async def _get():
        async with aiosqlite.connect(DB_NAME) as db:
            cursor = await db.execute(
