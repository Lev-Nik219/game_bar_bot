...
from database import (
    get_user, get_user_stats, update_balance, DB_NAME, get_bot_stats,
    claim_daily_bonus, get_bonus_total, set_user_started,
    log_agreement, increment_referral_count, increment_withdrawals_count,
    get_active_tournament, get_tournament_leaders
)
from keyboards import (
    profile_keyboard, main_reply_keyboard, deposit_keyboard,
    games_menu_keyboard, back_to_menu_keyboard, roulette_choice_keyboard,
    blackjack_keyboard, agreement_short_keyboard, agreement_keyboard,
...
async def reply_daily_bonus(message: types.Message):
    ...
    if success:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            async with db.execute(
                "SELECT daily_bonus_last FROM users WHERE user_id = ?", (user_id,)
            ) as cursor:
                row = await cursor.fetchone()
...
async def achievements_my_callback(callback: types.CallbackQuery):
    ...
    if rows:
        ...
        text = "🏆 Твои достижения:\n" + "\n".join(
            f"• {achievement_names.get(ach, ach)}" for ach in [r[0] for r in rows]
        )
    else:
...
async def withdraw_wallet(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "INSERT INTO withdraw_requests (user_id, amount_points, amount_usdt, wallet_address) "
            "VALUES (?, ?, ?, ?)",
