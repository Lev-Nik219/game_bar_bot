...
async def admin_stats_callback(callback: types.CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS: await callback.answer("❌ Доступ запрещён", show_alert=True); return
    await callback.message.edit_text("📊 <b>Статистика</b>\n\nВыберите тип:", parse_mode="HTML", reply_markup=admin_stats_keyboard()); await callback.answer()
...
async def admin_stats_main_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        c.execute("SELECT COALESCE(SUM(win_amount), 0) FROM game_history"); total_wins = c.fetchone()[0]
        conn.close()
        text = (
...
async def admin_broadcast_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📢 Введите сообщение для массовой рассылки ВСЕМ пользователям:", reply_markup=cancel_keyboard()); await callback.answer()
...
async def admin_cashback_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        await callback.message.edit_text(f"✅ <b>Кэшбек завершён!</b>\n\n👥 Пользователей: {users_notified}\n💎 Всего начислено: {total_cashback} баллов", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Вернуться в админ-панель", callback_data="admin_back")]]))
    except Exception as e:
        logger.error(f"Cashback error: {e}")
        await callback.message.edit_text(f"❌ Ошибка: {e}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Вернуться в админ-панель", callback_data="admin_back")]]))
